...
# Response iat (0  = negative, 1 = positive)
#
...
# model
iat_acc <- brm(correct ~ congruent * order * RWAscore + (1|ppt) + (1|stim),
                   data = iat_df, 
                   family = bernoulli(),
                   prior = priors,
...
# model_acc_xp04 <- summary(iat_acc)$fixed[,col2keep]
# model_acc_xp04 <- round(model_acc_xp04, 3)

# arrange output
iat_acc_xp04 <- tidy_stan(iat_acc,
                            typical = "mean",
                            prob = .95)


# export output
png("tables/brms/model_acc_xp04.png", height=480, width=720)
p<-tableGrob(model_acc_xp04)
grid.arrange(p)
dev.off()

# test with rope
equi_acc_xp04_rwa <- equi_test(iat_acc, rope = c(-0.01, 0.01))
equi_acc_xp04_rwa <- equi_acc_xp04_rwa[c(4,6:8),]
equi_acc_xp04_rwa$ROPE <- "-0.01, 0.01"

equi_acc_xp04_0.5 <- equi_test(iat_acc, rope = c(-0.05, 0.05))
equi_acc_xp04_0.5 <- equi_acc_xp04_0.5[c(2,3,5),]
equi_acc_xp04_0.5$ROPE <- "-0.05, 0.05"

equi_acc_xp04 <- rbind(equi_acc_xp04_rwa, equi_acc_xp04_0.5)
equi_acc_xp04[,c(3:5)] <- round(equi_acc_xp04[,c(3:5)], 2)
ordrow <- c(5, 6, 1, 7, 2, 3, 4)
equi_acc_xp04 <- equi_acc_xp04[ordrow,]

# export test
png("tables/rope/equi_acc_xp04.png", height=480, width=720)
p<-tableGrob(equi_acc_xp04)
grid.arrange(p)
dev.off()
