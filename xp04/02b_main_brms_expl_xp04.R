...
rrwa <- round(max ( diriat_df$RWAscore) - min ( diriat_df$RWAscore))
...
# model
diriat_resp <- brm(response ~ usvalence * order * RWAscore + (1|ppt) + (1|stim1),
                   data = diriat_df, 
...
# model_gen_xp04 <- summary(diriat_resp)$fixed[,col2keep]
# model_gen_xp04 <- round(model_gen_xp04, 3)

# arrange output
diriat_gen_xp04 <- tidy_stan(diriat_resp,
                            typical = "mean",
                            prob = .95)


# export output
png("tables/brms/diriat_gen_xp04.png", height=480, width=720)
p<-tableGrob(diriat_gen_xp04)
grid.arrange(p)
dev.off()

# test with rope
equi_gen_diriat__xp04_rwa <- equi_test(diriat_resp, rope = roperwaN)
equi_gen_diriat__xp04_rwa <- equi_gen_diriat__xp04_rwa[c(11,13:15),]
equi_gen_diriat__xp04_rwa$ROPE <- roperwaC

equi_gen_diriat__xp04_0.5 <- equi_test(diriat_resp, rope = c(-0.5, 0.5))
equi_gen_diriat__xp04_0.5 <- equi_gen_diriat__xp04_0.5[c(9,10,12),]
equi_gen_diriat__xp04_0.5$ROPE <- "-0.5, 0.5"

equi_gen_diriat__xp04 <- rbind(equi_gen_diriat__xp04_rwa, equi_gen_diriat__xp04_0.5)
equi_gen_diriat__xp04[,c(3:5)] <- round(equi_gen_diriat__xp04[,c(3:5)], 2)
ordrow <- c(5,6,1,7,2,3,4)
equi_gen_diriat__xp04 <- equi_gen_diriat__xp04[ordrow,]

# export test
png("tables/rope/equi_gen_diriat__xp04.png", height=480, width=720)
p<-tableGrob(equi_gen_diriat__xp04)
grid.arrange(p)
dev.off()
