...
dataint<- data.frame( value = c(posterior_samples(ec01rwaP, "b")$"b_rwa",
                                posterior_samples(ec02rwaP, "b")$"b_rwa",
                                posterior_samples(ec03rwaP, "b")$"b_rwa",
                                posterior_samples(ecmmrwaP, "b")$"b_rwa"
),
...
hmmrwus <- hypothesis (ecmmrwaP, class = "b", "rwa = 0")
bf0mmrwus <- 1
...
p1 <-
  ...
  geom_text(x=-0.7, y=1, data=subset(dataint, select == 1),
