...
dataint<- data.frame( value = c(posterior_samples(ec01rwa, "b")$"b_rwa:usvalence",
                                posterior_samples(ec02rwa, "b")$"b_rwa:usvalence",
                                posterior_samples(ec03rwa, "b")$"b_rwa:usvalence",
                                posterior_samples(ecmmrwa, "b")$"b_rwa:usvalence"
),
...
h01rwus <- hypothesis (ec01rwa, class = "b", "rwa:usvalence = 0")
bf001rwus <- 1
...
h02rwus <- hypothesis (ec02rwa, class = "b", "rwa:usvalence = 0")
bf002rwus <- 1
...
h03rwus <- hypothesis (ec03rwa, class = "b", "rwa:usvalence = 0")
bf003rwus <- 1
...
hmmrwus <- hypothesis (ecmmrwa, class = "b", "rwa:usvalence = 0")
bf0mmrwus <- 1
...
p1 <-
  ...
  labs(x = expression(beta[RWA%*%"Valence Block 1"]))+
  ...
  geom_text(x=0.12, y=1, data=subset(dataint, select == 1),
            aes(label=paste(round(value.ave,2),"[",
                            round(value.low,2),", ",round(value.up,2),"]")), size = 2.5)+
  geom_segment(aes(x = min(value), xend = max(value), y = 8,
                   yend = 8, colour = sep), size = 0.2)+
