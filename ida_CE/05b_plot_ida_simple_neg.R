...
datIDAneg <- data.frame( value = c(posterior_samples(XP03_resp_neg, "b")$"b_RWAscore",
                                posterior_samples(XP10_resp_neg, "b")$"b_RWAscore",
...
hdi_neg10 <- HDInterval::hdi(posterior_samples(XP10_resp_neg, "b")$"b_RWAscore",
                         credMass = 0.95)
hdi_neg10m <- mean(posterior_samples(XP10_resp_neg, "b")$"b_RWAscore",
               credMass = 0.95)
...
plot_IDAneg <- datIDAneg %>% 
  ...
  #annotate("rect", xmin = -0.25, xmax = 0.33, ymin = 1.945, ymax = 1.955, fill = "gray30", alpha = 0.7) +
  annotate("errorbarh", y = 3, xmin = hdi_neg03[1], xmax = hdi_neg03[2], colour = "grey10", size = 0.6, height = 0) +
  annotate("point", y = 3, x = hdi_neg03m, colour = "grey10", size = 1.2) +
  annotate("errorbarh", y = 2, xmin = hdi_neg10[1], xmax = hdi_neg10[2], colour = "grey10", size = 0.6, height = 0) +
  annotate("point", y = 2, x = hdi_neg10m, colour = "grey10", size = 1.2) +
  ...
  annotate("text", label = paste("N = ", nbppt03), x = 0.35, y = 03, colour = "grey10", size = 3.2, family = font_rc_light) +
  annotate("text", label = paste("N = ", nbppt10), x = 0.35, y = 02, colour = "grey10", size = 3.2, family = font_rc_light) +
  annotate("text", label = paste("N = ", nbpptIDA), x = 0.35, y = 01, colour = "grey10", size = 3.2, family = font_rc_light) +
  geom_vline(xintercept = 0, linetype="dashed", color = "red") + 
  labs(x = expression(beta[RWA["Negative EC"]]),
       y = "Experiment")+
  scale_x_continuous(breaks=seq(-0.25, 0.35, 0.1)) +
...
plot_IDAneg
ggsave("plots/forest_simple_neg_EC_eng.jpg", width = 35, height = 20, units = "cm")
ggsave("plots/forest_simple_neg_EC_eng.png", width = 35, height = 20, units = "cm")
ggsave("plots/forest_simple_neg_EC_eng.pdf", width = 35, height = 20, units = "cm")
