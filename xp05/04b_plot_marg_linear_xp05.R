...
# !!One counter-conditioning!!
marg_ordone <- marginal_effects(expli_resp, effects = "RWAscore:usvalence", 
                            conditions = cond_ordone, method = c("fitted"), # here his where we specify the combination
                            re_formula = NULL,
                            spaghetti = TRUE, nsamples = 500)

# !!Two counter-conditionings!!
marg_ordtwo <- marginal_effects(expli_resp, effects = "RWAscore:usvalence", 
...
attr(marg_ordone$`RWAscore:usvalence`, "spaghetti") <- attr(marg_ordone$`RWAscore:usvalence`, "spaghetti")[grepl(0.5, attr(marg_ordone$`RWAscore:usvalence`, "spaghetti")$sample__),]

attr(marg_ordtwo$`RWAscore:usvalence`, "spaghetti") <- attr(marg_ordtwo$`RWAscore:usvalence`, "spaghetti")[grepl(0.5, attr(marg_ordtwo$`RWAscore:usvalence`, "spaghetti")$sample__),]

attr(marg_ordone$`RWAscore:usvalence`, "spaghetti")$usvalence <- ifelse (attr(marg_ordone$`RWAscore:usvalence`, "spaghetti")$usvalence == -0.5, "Négative", "Positive")

attr(marg_ordtwo$`RWAscore:usvalence`, "spaghetti")$usvalence <- ifelse (attr(marg_ordtwo$`RWAscore:usvalence`, "spaghetti")$usvalence == -0.5, "Négative", "Positive")
...
# marg_spag_all
# ggsave("plots/marg_spag_xp05_french.jpg", width = 50, height = 15, units = "cm")
# ggsave("plots/marg_spag_xp05_french.pdf", width = 50, height = 15, units = "cm")
# ggsave("plots/marg_spag_xp05_french.tex", width = 50, height = 15, units = "cm")
