...
# model
apt_resp_lme4 <- lmer(RT ~ congruent * order * RWAscore + (1|ppt) + (1|stim1),
                   data = apt_df)

# Save summary & confint
apt_gen_xp05_lme4 <- round(cbind(summary(apt_resp_lme4)$coefficients,
                             confint(apt_resp_lme4)[c(4:11),]), 2)

# export output
png("tables/lme4/apt_gen_xp05_lme4.png", height=480, width=720)
p<-tableGrob(apt_gen_xp05_lme4)
grid.arrange(p)
dev.off()
