...
attr(marg_b1$`RWAscore:usvalence2`, "spaghetti")$usvalence2 <- ifelse (attr(marg_b1$`RWAscore:usvalence2`, "spaghetti")$usvalence2 == -0.5, "Negative", "Positive")

attr(marg_b2$`RWAscore:usvalence2`, "spaghetti")$usvalence2 <- ifelse (attr(marg_b2$`RWAscore:usvalence2`, "spaghetti")$usvalence2 == -0.5, "Negative", "Positive")
...
marg_plot_b1 = plot(marg_b1, plot = FALSE, mean = F)[[1]] + # here his where we specify the marginal effects of interest
  ...
  labs(colour="LAI",
       title="Simple conditioning") + # here his where we mention the marginal effects of interest
...
marg_plot_b2 = plot(marg_b2, plot = FALSE, mean = F)[[1]] + # here his where we specify the marginal effects of interest
  ...
  scale_x_continuous(name="RWA", breaks = scales::pretty_breaks(n = 10), expand=c(0.01,0)) +
  labs(colour="LAI",
       title="Conditioning and counter conditioning") + # here his where we mention the marginal effects of interest
  theme_ipsum_rc(base_size = 13,
                 subtitle_size = 20,
                 axis_title_size = 15) +
  guides(color=guide_legend(override.aes=list(fill=NA)))
...
# marg_spag_all
# ggsave("plots/marg_spag_xp11_english.jpg", width = 50, height = 15, units = "cm")
# ggsave("plots/marg_spag_xp11_english.pdf", width = 50, height = 15, units = "cm")
# ggsave("plots/marg_spag_xp11_english.tex", width = 50, height = 15, units = "cm")
