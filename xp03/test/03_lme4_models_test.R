...
# model
bloc_resp_lme42 <- lmer(response ~ usvalence * bloc * RWAscore + (1|ppt) + (1|stim1),
                         data = bloc_df2)

# Save summary & confint
model_gen_xp02_lme4 <- round(cbind(summary(bloc_resp_lme42)$coefficients,
                                   confint(bloc_resp_lme42)[c(4:11),]), 2)
...
# model
bloc_resp_neg_lme4 <- lmer(response ~ usvalence_neg * RWAscore + (1|ppt) + (1|stim1),
                               data = bloc_df)

# Save summary & confint
model_neg_xp02_lme4 <- round(cbind(summary(bloc_resp_neg_lme4)$coefficients,
                                     confint(bloc_resp_neg_lme4)[c(4:7),]), 2)
...
# model
bloc_resp_pos_lme4 <- lmer(response ~ usvalence_pos * RWAscore + (1|ppt) + (1|stim1),
                               data = bloc_df)

# Save summary & confint
model_pos_xp02_lme4 <- round(cbind(summary(bloc_resp_pos_lme4)$coefficients,
                                     confint(bloc_resp_pos_lme4)[c(4:7),]), 2)
